...
sub comments {
 ...
 # Remove everything after a left parenthesis '('
        $_ ~~ s/ '('.* //;

        # Remove everything after a left bracket '['
        $_ ~~ s/ '[' .* //;

        # Remove everything after a left curly brace '{'
        $_ ~~ s/ '{' .* //;

        # Remove everything after a backslash '\'
        $_ ~~ s/ \\ .* //;

        # Remove everything after a forward slash '/'
        $_ ~~ s/ \/ .* //;

        # Remove everything after an underscore '_'
        $_ ~~ s/ _ .* //;
        ...
        # Remove everything after a quotation mark '"'
        $_ ~~ s/ '"' .* //;

        # Remove everything after a backtick '`'
        $_ ~~ s/ \` .* //;
        ...
        # Remove everything after the literal 'feat.' (featuring)
        $_ ~~ s/ feat\. .* //;
...
sub clean {
	if ($DEBUG) {
		say "<filtering non-ASCII characters>\n";}
	my @filteredtitles = ();
...
sub stopwords {
	...
	##########################
	for @tracks -> $title {
        # Assign the current title to the default variable $_ for manipulation
		##########################
		$_ = $title;
    ...
    for @tracks -> $title  {
        ...
        if (@words > 1) {
            ...
            for 0 ..^ @words.elems - 1 -> $i {
                ...
                # Update the count for the word pair in %counts
                %counts{$word1}{$word2}++;
        }
	}
	}
...
sub mcw {
	...
	##########################
	my $max_count = 0; # Declare max_count at the start of the function
    ...
    for @next_words -> $next_word {
        # Check if the candidate word has been used before
        next if %word_history{$next_word}:exists;

        # Update the best word if this candidate has a higher count
...
sub sequence {
    ...
    # Track the initial word
    %word_history{$current_word} = 1;
    my @song_title = $current_word;

    # Iterate until the maximum sequence length is reached or the next word is empty
    while @song_title.elems < $SEQUENCE_LENGTH {
        my $next_word = mcw($current_word);

    
        # Break if there is no valid next word
        last if $next_word eq '' || $next_word ~~ /\n/;
        %word_history{$next_word} = 1;

        # Add the next word to the song title and update word history
        push @song_title, $next_word;
	...
	return @song_title.join(" ");  # Joins without any spaces or separators
	########################## End Task Song Title
}
